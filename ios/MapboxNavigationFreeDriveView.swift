...
class MapboxNavigationFreeDriveView: UIView, NavigationMapViewDelegate, NavigationViewControllerDelegate {
  ...
  func startActiveGuidance(updateCamera: Bool) {
    ...
    if (response != nil) {
      ...
      navigationMapView.mapView.mapboxMap.onNext(event: .styleLoaded, handler: { [weak self] _ in
        ...
        var layerExists = self.navigationMapView.mapView.mapboxMap.style.layerExists(withId: "road-intersection")
  ...
  func showSpeedLimitView() {
    ...
    if (instructionsCardContainerView.isHidden) {
      speedLimitView.topAnchor.constraint(equalToConstant: speedLimitAnchor.indices.contains(0) ? CGFloat(speedLimitAnchor[0].floatValue) : 10).isActive = true
    } else {
    ...
    speedLimitView.leadingAnchor.constraint(equalToConstant: speedLimitAnchor.indices.contains(1) ? CGFloat(speedLimitAnchor[1].floatValue) : 10).isActive = true
  }
